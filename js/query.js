function parsePlanetName() {
    ...
    let planetName = $('#select2-planet-names').val();
...
function getPlanetComposition() {
  ...
  SELECT DISTINCT ?atmosphere WHERE {
    ...
    const encodedPlanetCompositionQuery = buildUrlWithQuery(
        planetCompositionQuery
    );
        ...
        success: function(result) {
            ...
            if (atmospheres !== false) {
                atmospheres.forEach(function show(key, values) {
                    try {
                        // Try because DBPedia doesn't send consistent data, so type might not be present
                        getLabelFromUri(values, function(gaz) {
                            ...
                            document.getElementById(
                                'composition'
                            ).innerHTML = composition;
                        });
                    } catch (e) {}
                });
            } else {
                document.getElementById('composition').innerHTML =
                    'No composition info found';
            }
        },
        error: function(error) {
            console.log(error);
...
function getLabelFromUri(uri, callback) {
    ...
    var uriToLabelQuery = `select  ?label
  where {
  ...
  LIMIT 1`;
    ...
    var encodeduriToLabelQuery =
        baseURL + encodeURI(uriToLabelQuery) + queryParams;
    try {
        $.ajax({
...
function getThumbnailImage() {
    ...
    const encodedThumbnailQuery =
        'https://query.wikidata.org/sparql?query=SELECT%20%3Fplanet%20%3FplanetLabel%20%3Fimage%20%3Flieu%20%3FlieuLabel%20WHERE%20%7B%0A%20%20%3Fplanet%20wdt%3AP18%20%3Fimage%3B%0A%20%20%20%20rdfs%3Alabel%20%3FplanetLabel%3B%0A%20%20%20%20wdt%3AP276%20wd%3AQ7879772.%0A%20%20FILTER(STRSTARTS(%3FplanetLabel%2C%20%22' +
        planetName +
        '%22))%0A%7D&format=json';
